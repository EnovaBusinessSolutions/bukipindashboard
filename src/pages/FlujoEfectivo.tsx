...
/**
 * 🔒 Contexto para que TODOS los componentes del panel usen el mismo rango
 * en formato canonical YYYY-MM-DD (evita URLs rotas y 404).
 */
type FlujoRangeContextValue = {
  startDate: Date;
  endDate: Date;
  startYMD: string; // "2026-01-01"
  endYMD: string;   // "2026-01-31"
  periodType: PeriodType;
  vistaColumnas: "consolidada" | "detallada";
  filtroMetodoPago: "consolidado" | "efectivo" | "bancos";
};

const FlujoEfectivoRangeContext = createContext<FlujoRangeContextValue | null>(null);

export const useFlujoEfectivoRange = () => {
  const ctx = useContext(FlujoEfectivoRangeContext);
  if (!ctx) {
    throw new Error("useFlujoEfectivoRange debe usarse dentro de <FlujoEfectivo />");
  }
  return ctx;
};
...
const FlujoEfectivo = () => {
  ...
  // ✅ Canonical YYYY-MM-DD (esto es lo que debe ir al backend)
  const startYMD = useMemo(() => format(dateRange.startDate, "yyyy-MM-dd"), [dateRange.startDate]);
  const endYMD = useMemo(() => format(dateRange.endDate, "yyyy-MM-dd"), [dateRange.endDate]);

  const ctxValue = useMemo<FlujoRangeContextValue>(
    () => ({
    ...
    [dateRange.startDate, dateRange.endDate, startYMD, endYMD, periodType, vistaColumnas, filtroMetodoPago]
  );

  return (
    <FlujoEfectivoRangeContext.Provider value={ctxValue}>
    ...
    </FlujoEfectivoRangeContext.Provider>
  );
};
